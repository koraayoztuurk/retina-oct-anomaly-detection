...
FINAL_RUN_ID = "ae_mse_l128_e60_plateau_bn"
FIXED_LR_RUN_ID = "ae_mse_l128_e60"
SCHEDULER_RUN_ID = "ae_mse_l128_e60_plateau"
...
def create_results_figure(data: dict, path: Path) -> None:
    ...
    scheduler_row = ledger[
        (ledger["category"] == "score_ablation_image_level")
        & (ledger["run_id"] == SCHEDULER_RUN_ID)
        & (ledger["score_mode"] == FINAL_SCORE_MODE)
    ].iloc[0]
    ...
    labels = ["AE-MSE\n40e", "VAE+KL", "Top-k\n60e+LR", "Top-k\nLR+BN"]
    aurocs = [baseline["auroc"], vae["auroc"], scheduler_row["auroc"], final_row["auroc"]]
    f1s = [baseline["f1"], vae["f1"], scheduler_row["f1"], final_row["f1"]]
    x = range(len(labels))
...
def make_tables(data: dict) -> dict[str, pd.DataFrame]:
    ...
    main_rows = [
        ...
        ("AE-MSE 60e + LR", row("training_run", SCHEDULER_RUN_ID), "Scheduler; raw MSE'de küçük artış"),
        ("AE top-k 60e + LR", row("score_ablation_image_level", SCHEDULER_RUN_ID, FINAL_SCORE_MODE), "Önceki final; DRUSEN 102/250"),
        ("AE-MSE LR+BN", row("training_run", FINAL_RUN_ID), "BatchNorm; val loss ve raw MSE arttı"),
        ("AE top-k LR+BN", row("score_ablation_image_level", FINAL_RUN_ID, FINAL_SCORE_MODE), "Final aday; DRUSEN 107/250"),
    ...
    setup = pd.DataFrame(
        [
            ...
            {"Ayar": "BatchNorm", "Değer": "encoder ve decoder bloklarında Conv/ConvTranspose sonrası aktif"},
...
def report_sections(data: dict, tables: dict[str, pd.DataFrame], gpu: str) -> dict[str, str]:
    ...
    return {
        "Özet": (
            ...
            "baseline korunmuş; VAE+KL, L1, MSE+SSIM, latent boyut, batch size, crop ön işleme, learning rate scheduler, BatchNorm, farklı anomaly score'lar, hasta düzeyi "
            "birleştirme ve bootstrap güven aralığı denemeleri eklenmiştir. En iyi image-level sonuç, BatchNorm ve ReduceLROnPlateau ile 60 epoch eğitilen AE-MSE modelinin "
        ...
        "Abstract": (
            ...
            "VAE+KL, L1, MSE+SSIM, latent-size and batch-size ablations, crop preprocessing trials, learning-rate scheduling, BatchNorm, alternative anomaly scores, patient-level aggregation and bootstrap confidence intervals. "
            f"The best image-level setting was the 60-epoch AE-MSE model trained with BatchNorm and ReduceLROnPlateau and evaluated with topk_mse_5 scoring, reaching AUROC {best_image['auroc']:.4f}, F1 {best_image['f1']:.4f}, "
        ...
        "Giriş": (
            ...
            "Bu kapsamda VAE tabanlı üretken latent model, L1 ve SSIM içeren loss seçenekleri, latent boyut ve batch size ablasyonları, learning rate scheduler, BatchNorm, boş alanları azaltmaya yönelik crop denemeleri, top-k residual score, hasta düzeyi değerlendirme, "
        ...
        "Bulgular": (
            ...
            "Confusion matrix düzeyinde image-level değerlendirmede TN=242, FP=8, FN=179 ve TP=571; patient-level değerlendirmede ise TN=158, FP=13, FN=75 ve TP=439 elde edilmiştir. "
            "Tablo VII, p95-p97-p99 eşiğinin precision-recall dengesini nasıl değiştirdiğini göstermektedir. p97 ve p99 precision değerini korusa da recall belirgin biçimde düştüğü için final operasyon noktası p95 olarak bırakılmıştır. Tablo VIII, sınıf bazlı davranışı göstermektedir. CNV 248/250, DME 216/250 yakalanırken DRUSEN 107/250 seviyesine çıkmıştır. Bu durum BatchNorm, scheduler ve top-k score birleşiminin DRUSEN tarafında küçük ama ölçülebilir bir kazanım sağladığını, ancak DRUSEN'in normal anatomiye yakın ve daha lokal sapmalar içerebildiği için halen en zor sınıf olduğunu göstermektedir. Üretilen heatmap ve DRUSEN false negative görselleri de modelin bazı DRUSEN örneklerinde patolojik bölgeyi yeterince yüksek score'a taşıyamadığını göstermiştir."
            "\n\n"
...
def add_dataframe_table(doc: Document, frame: pd.DataFrame, caption: str, font_size: float = 8.0) -> None:
    ...
    set_table_margins(table, margin_twips=30)
...
def build_docx(data: dict, tables: dict[str, pd.DataFrame], docx_path: Path) -> None:
    ...
    add_figure(doc, FINAL_FIGURE_DIR / "final_pipeline.png", "Şekil 1. Önerilen final deney hattı.", width=2.85)
    ...
    add_figure(doc, FINAL_FIGURE_DIR / "final_results_summary.png", "Şekil 2. Final performans ve sınıf bazlı tespit özeti.", width=2.85)
    add_figure(
        doc,
